    ...
    public class Markdown
    {
        ...
        private static IList<string> IsolateAndParseCode(ref string content)
        {
            ...
            for (int i = 0; i < codeChunks.Count; i++)
            {
                ...
                codeChunks[i] = Regex.Replace(codeChunks[i], @"^%%LANG.*?%<br/>", m => string.Empty);
        ...
        private static void LineBreaksFirstPass(ref string content)
        {
            ...
            content = Regex.Replace(content, @"(<br/>)+(?=[^\w\d]*?```)", m => string.Empty, RegexOptions.Multiline | RegexOptions.Singleline); // lines before code
            content = Regex.Replace(content, @"(<br/>)+(?=[^\w\d]*?>\s)", m => string.Empty, RegexOptions.Multiline | RegexOptions.Singleline); // lines before quotes
        }

        private static void LineBreaksSecondPass(ref string content)
        {
            content = content.Replace("\r\n", "\n");
            content = Regex.Replace(content, @"(<br/>)+(?=[^\w\d]*?<table>)", m => string.Empty, RegexOptions.Multiline | RegexOptions.Singleline); // lines before tables
            content = Regex.Replace(content, @"(<br/>)+(?=[^\w\d]*?<ul>)", m => string.Empty, RegexOptions.Multiline | RegexOptions.Singleline); // lines before ul
            content = Regex.Replace(content, @"(<br/>)+(?=[^\w\d]*?<ol>)", m => string.Empty, RegexOptions.Multiline | RegexOptions.Singleline); // lines before ol
            content = Regex.Replace(content, @"(?<=<li>.*)(<br/>)+(?=.*</li>)", m => string.Empty, RegexOptions.Multiline); // lines in li
            content = Regex.Replace(content, @"(<br/>)+(?=</blockquote>)", m => string.Empty, RegexOptions.Multiline); // lines in quotes
            content = Regex.Replace(content, @"(<br/>)+(?=</h[123456]>)", m => string.Empty, RegexOptions.Multiline); // lines in h
            content = Regex.Replace(content, @"(<br/>)+(?=\s*?<h[123456]>)", m => string.Empty, RegexOptions.Multiline); // lines before h
        ...
        private static void ParseHeaders(ref string content)
        {
            for (int i = 1; i <= 6; ++i)
            {
                content = Regex.Replace(content, $@"^#{{{i}}}[^#](?<text>.+?)$", m => $"<h{i} id=\"{m.Groups["text"].Value.TrimEnd('\r').TrimEnd('\n').Replace(' ', '-').ToLowerInvariant()}\">{m.Groups["text"].Value.TrimEnd('\r').TrimEnd('\n')}</h{i}>", RegexOptions.Multiline);
            }
        }

        private static void ParseHorizontalRules(ref string content)
        {
            content = Regex.Replace(content, @"^[\*-_]{3,}$", m => "<hr>", RegexOptions.Multiline);
        }
        ...
        private static void ParseTables(ref string content)
        {
            ...
            System.Collections.Generic.List<string> tableHeader = new System.Collections.Generic.List<string>();
